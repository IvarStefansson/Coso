...
from porepy.examples.geothermal_reservoir import (
    NeumannWellBCsFirstTimeInterval, WellBoundaryConditions)
...
from geometry import LargeGeometry
...
if __name__ == "__main__":
    ...
    for strike in fracture_strike_angles:
        for has_solid_expansion in [True]:
            tic = time.time()
            if has_solid_expansion:
                suffix = "_with_solid_expansion"
            else:
                suffix = "_without_solid_expansion"
                granodiorite_values["thermal_expansion"] = 0.0
                init_granodiorite_values["thermal_expansion"] = 0.0
            ...
            schedule = (
                np.array([0, 1, 10, 20, 50, 80, 111, 112, 113, 114, 115, 116]) * pp.DAY
            )
            schedule = np.array(
                [
                    0,
            ...
            schedule = (
                np.array(
                    [
                        ...
                        1 * pp.DAY,  # Ramp up to operation
                        2 * pp.YEAR - 2 * pp.DAY,  # First shut-in
                        2 * pp.YEAR,  # Restart operation
                        4 * pp.YEAR - 2 * pp.DAY,  # Second shut-in
                        4 * pp.YEAR,  # End second shut-in
                        6 * pp.YEAR - 2 * pp.DAY,  # Third shut-in
                        6 * pp.YEAR,
                        8 * pp.YEAR - 2 * pp.DAY,  # First shut-in
                        8 * pp.YEAR,  # Restart operation
                        10 * pp.YEAR - 2 * pp.DAY,  # Second shut-in
                        10 * pp.YEAR,  # End second shut-in
                        12 * pp.YEAR - 2 * pp.DAY,  # Third shut-in
                        12 * pp.YEAR,  # End of simulation
                    ]
                )
                * 2
            )
            ...
            #     (schedule[4], schedule[5]),  # Shut-in period 2
            #     (schedule[6], schedule[7]),  # Shut-in period 3
            #     (schedule[8], schedule[9]),  # Shut-in period 4
            #     (schedule[10], schedule[11]),  # Shut-in period 5
            # ]
            neumann_intervals = [
            ...
            file_name = "example_5"
            model_params_init = {
                "domain_sizes": np.array([14, 14, 4]) * 1e3,
                "material_constants": {
                ...
                "fracture_params": {  # Other options are available in the geometry mixin.
                    ...
                    "strike_angle": np.deg2rad(strike),
                },
            }
