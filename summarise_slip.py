...
if __name__ == "__main__":
    ...
    for strike in fracture_strike_angles:
        ...
        for angle_index in angle_indices:
            ...
            slip_status[strike][angle_index] = {
                "with_wells": None,
                "without_wells": None,
            }
            ...
            csv_with_wells = (
                Path(f"case_II_with_wells{suffix}_saved_data")
                / "well_monitoring"
                / "example_7_fractures.csv"
            )
            ...
            # Check without wells
            csv_without_wells = (
                Path(f"case_II_without_wells{suffix}_saved_data")
                / "well_monitoring"
                / "example_7_fractures.csv"
            )
            ...
            if slip_with is not None or slip_without is not None:
                print(
                    f"Strike {strike}°, Angle Index {angle_index} (Fracture {fracture_num}):"
                )
    ...
    if 45 in slip_status:
        ...
        for angle_index in angle_indices:
            ...
            print(
                f"Strike 45°, Angle Index {angle_index}: Using data from angle index 0"
            )
    ...
    header = (
        "Strike° | " + " | ".join([f"Angle Index {i}" for i in angle_indices]) + " |"
    )
...
"""
    ...
    output_file = Path("slip_summary_table.html")
    with output_file.open("w") as f:
        ...
        f.write("<ul>\n")
        f.write(
            '  <li><span style="background-color: #90EE90; padding: 2px 10px;">Green</span>: Slip both with and without wells</li>\n'
        )
        f.write(
            '  <li><span style="background-color: #FFFF66; padding: 2px 10px;">Yellow</span>: Slip only with wells</li>\n'
        )
        f.write(
            '  <li><span style="background-color: #FF6B6B; padding: 2px 10px;">Red</span>: No slip</li>\n'
        )
        f.write(
            '  <li><span style="background-color: #ffffff; padding: 2px 10px; border: 1px solid #ccc;">White</span>: Missing data</li>\n'
        )
        f.write("</ul>\n")
        f.write(html)
        f.write("</body></html>")
