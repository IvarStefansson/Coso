    ...
    def collect_data(self) -> dict:
        ...
        for id, sd in enumerate(sds):
            if not self.is_well(sd):
                ...
                continue
            f = self.domain_boundary_sides(sd).top
            if sum(f) == 0:
            ...
            flux_sign = np.sign(sd.face_normals[-1, f])

            data[parent_well.tags["well_name"]] = {
                "pressure": pressure[face_offsets[id] : face_offsets[id + 1]][f][0],
                "temperature": temperature[face_offsets[id] : face_offsets[id + 1]][f][
                    0
                ],
                "darcy_flux": darcy_f[face_offsets[id] : face_offsets[id + 1]][f][0]
                * flux_sign,
                "enthalpy_flux": enthalpy_f[face_offsets[id] : face_offsets[id + 1]][f][
                    0
                ]
                * flux_sign,
                "fluid_flux": (
                    fluid_f[face_offsets[id] : face_offsets[id + 1]][f][0] * flux_sign
                ),
            }
        return data
